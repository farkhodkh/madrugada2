...
/** TODO - Тех долг, нужно все перенести на kotlin */
public class BerTlvParser {
    ...
    private ParseResult parseWithResult(int aLevel, byte[] aBuf, int aOffset, int aLen) {
        ...
        if (aOffset + aLen > aBuf.length) {
            throw new IllegalStateException(
                    "Length is out of the range [offset="
                            + aOffset
                            + ",  len="
                            + aLen
                            + ", array.length="
        ...
        if (tag.isConstructed()) {
            ...
            System.arraycopy(
                    aBuf, aOffset + tagBytesCount + lengthBytesCount, value, 0, valueLength);
            int resultOffset = aOffset + tagBytesCount + lengthBytesCount + valueLength;
            if (log.isDebugEnabled()) {
    ...
    private void addChildren(
        ...
        while (startPosition < aOffset + valueLength) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "{}level {}: adding {} with offset {}, startPosition={}, aDataBytesCount={}, valueLength={}",
                        levelPadding,
                        aLevel,
                        result.tlv.getTag(),
                        result.offset,
    ...
    private static final IBerTlvLogger EMPTY_LOGGER =
            new IBerTlvLogger() {
